...
class PHPWord_Writer_Word2007 implements PHPWord_Writer_IWriter {
		...
		if(!is_null($this->_document)) {
			...
			foreach($_sections as $section) {
				...
				if(!is_null($_header)) {
					...
					$sectionElements[] = ['target' =>$_headerFile, 'type' =>'header', 'rID' =>$rID];
				...
				if(!is_null($_footer)) {
					...
					$sectionElements[] = ['target' =>$_footerFile, 'type' =>'footer', 'rID' =>$rID];
	...
	private function _chkContentTypes($src) {
		...
		$_supportedImageTypes = ['jpg', 'jpeg', 'gif', 'png', 'bmp', 'tif', 'tiff', 'php'];
