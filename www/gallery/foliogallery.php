...
<script type="application/javascript">
    $(document).ready(function () { // load colorbox settings
        $('.folioGallery').each(function () {
            ...
            $('#' + targetDiv + ' .vid').colorbox({rel: targetDiv, iframe: true, width: "200%", height: "200%"});
        });
    });
</script>
<?php
...
require_once '../../app/libs/Loader.php';
...
$extensions = array("jpg", "png", "gif", 'bmp', "JPG", "PNG", "GIF", 'BMP'); // allowed extensions in photo Gallery
$itemsPerPage = '30';    // number of images per page if not already specified in ajax mode
...
$num_captions_chars = '30';    // number of characters displayed in album and thumb captions
...
function make_thumb($folder, $file, $dest, $thumb_width, $extensions)
{
    ...
    if (in_array($file->getExtension(), $extensions)) {
        try{
            ...
            $width = $image->width;
            $height = $image->height;
            $image->resize(150, null, \Nette\Utils\Image::SHRINK_ONLY);
            $image->sharpen();
            $image->save($dest, 80);
        }catch (\Exception $e){
            //todo zalogovat
    ...
    /*	if ($width < $thumb_width) // if original image is smaller don't resize it
        ...
        if ($file->getExtension() == "gif" or $file->getExtension() == "png") // preserve transparency
        {
...
<div class="fg">
    ...
    if (empty($_REQUEST['album'])) // if no album requested, show all albums
    {
        ...
        if ($numAlbums == 0) { ?>
            ...
            <div class="titlebar"><p>Nebylo nalezeno žádné album</p></div>
            ...
            <div class="p10-lr">
                <u>Celkový počet alb:</u> <b><?php echo $numAlbums; ?></b>
            </div>
                    ...
                    <div class="thumb-wrapper">
                        <div class="thumb">
                            <a class="showAlb" rel="<?php echo $albums[$i]; ?>"
                               href="<?php echo $_SERVER['PHP_SELF']; ?>?album=<?php echo urlencode($albums[$i]); ?>">
    ...
    } else //display photos in album
    {
        ...
        if (!in_array($requested_album, $albums_in_maindir)) { ?>
            <span class="title">Fotogalerie &raquo; <a href="<?php echo $album_page_url; ?>"
                                                       class="refresh">Zpět</a></span>
            <p></p>
        ...
        /*
         * @todo dodelat - nyni to pada na timeoutu
        $cmd = '.\editGallery.cmd';
        file_put_contents('galleryPath.txt', serialize($album));
        exec($cmd);
        */
        ...
        <div class="p10-lr">
            <?php if ($fullAlbum == 1) { ?>
                <span class="title"><a href="<?php echo $album_page_url; ?>"
                                       style='color:black;font-size:15px; text-decoration: underline; text-align:center;margin-right:100px'
                                       ;>Zpět</a></span>
            <?php } ?>
            <span
                style='color:black;font-size:15px; font-family: Tahoma; text-align:right'><?php echo ucfirst($requested_album) . ' (' . $numFiles . ')' ?>
        </div>
        ...
        if ($numFiles == 0) { ?>
            ...
            <div class="p10-lr"><p>Album neobsahuje žádné fotografie.</p></div>
                ...
                if (isset($files[$i]) && is_file($album . '/' . $files[$i])) {
                    ...
                    if (in_array($ext, $extensions)) {
                        ...
                        if (!file_exists($thumb)) {
                            make_thumb($album, $files[$i], $thumb, $thumb_width, $extensions);
                        }

                        if ($i < $start || $i >= $start + $numPerPage) { ?><div style="display:none;"><?php } ?>
                        <div class="thumb-wrapper">
                            <div class="thumb">
                                <?php echo $thumb_url_start; ?><img src="<?php echo $thumb; ?>"
                                                                    alt="<?php echo $files[$i]; ?>"/><?php echo $thumb_url_end; ?>
                            </div>
                            <?php if ($show_captions) { ?>
                                <div class="caption">
                                    <?php if ($num_chars > $num_captions_chars) { ?>
                                        <div class="tooltip-container"><?php echo $caption; ?><span
                                                class="tooltip"><?php echo $full_caption; ?></span></div>
                                    <?php } else {
                                        echo $caption;
                                    } ?>
                                </div>
